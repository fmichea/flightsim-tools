...
const setup = () => mountHookWithStore(() => useChecklistData({ checklistName: Checklists.WORKINGTITLE_CJ4 }));

describe('useChecklistData', () => {
    test('checklist data is retrieved', () => {
        const { getCurrentHookResult } = setup();

        expect(getCurrentHookResult()).toEqual(ChecklistsData[Checklists.WORKINGTITLE_CJ4]);
    });
});
